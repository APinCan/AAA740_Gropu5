    ...
    for epoch in range(args.shared_epoch):
        for iter_idx, (imgs, _) in enumerate(train_loader):
            ...
            if step % args.n_critic == 0:
                ...
                fake_validity = dis_net(gen_imgs).detach()
...
def validate(args, fixed_z, gen_net: nn.Module, writer_dict, clean_dir=True):
    print('==== funcitons.py/validation runs!!! ====')
    writer = writer_dict["writer"]
    ...
    # get inception score
    logger.info("=> calculate inception score")
    mean, std = ist.get_IS(img_list)
    print(f"Inception score: {mean}")

    # get fid score
    logger.info("=> calculate fid score")
    fid_score = fst.calculate_fid_given_paths(fid_buffer_dir, fid_stat)
